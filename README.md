...
- **QualityCritic & Self-Correction Agent Loop**: When tailoring resumes and cover letters, the system runs a multi-agent self-correction loop. A `QualityCritic` agent audits the output generated by the `TailoringExpert` for grounding/hallucinations and job requirements alignment. If any issues are found, it provides detailed feedback and automatically prompts self-correction (up to 3 turns) until the materials are fully verified.
- **Recruiter Q&A Memory Database**: Form auto-filling automatically handles custom/recruiter questions. It searches a local database of past questions using Jaccard word similarity to retrieve verified answers. If no match is found, the system dynamically drafts a tailored answer using the LLM and stores it as an unverified draft in the Q&A Memory dashboard for you to review and verify.
- **Agent Run Logs**: Every step taken by individual sub-agents is recorded in a dashboard-accessible run log, providing complete transparency into the AI's reasoning and actions.
- **Dynamic LLM Provider Management Dashboard**: Manage and prioritize API configuration credentials for Google Gemini, OpenAI, Anthropic, and custom endpoints from the web settings panel.
- **Auto-Cooldown & Quota Detection (Credit Checker)**: Automatically handles rate-limit and quota limit exhaustion errors (`insufficient_quota`), putting affected providers on a 1-hour cooldown while seamlessly falling back to available alternative providers.
- **Visual Resume Customization Engine**: Tweak resume styling dynamically from your preference panel, specifying custom document margins, line heights, font sizes, and typography themes.
- **Multi-Profile Scenarios**: Explicitly associate specific job discoveries and target applications to multiple candidate profiles/personas to track different career tracks simultaneously.
